...
class Trainer(object):
    ...
    def val_epoch(self, epoch, weight, val_dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                output, output1 = self.model(data, target, teacher_forcing_ratio=0.)
    ...
    def train_epoch(self, epoch, weight):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            if self.args.real_value:
                label = self.scaler.inverse_transform(label)
                #label1 = self.scaler.inverse_transform(label1)
                label1 = torch.tensor(label1,dtype=torch.long)
            ...
            if batch_idx % self.args.log_step == 0:
                self.logger.info('Train Epoch {}: {}/{} Loss1: {:.6f} loss2: {:.6f}  loss: {:.6f}'.format(
                    epoch, batch_idx, self.train_per_epoch, loss1.item(), loss2.item(), loss.item()))
        train_epoch_loss = total_loss/self.train_per_epoch
        train_epoch_loss1 = total_loss1/self.train_per_epoch
        train_epoch_loss2 = total_loss2/self.train_per_epoch
        self.logger.info('**********Train Epoch {}: averaged loss1: {:.6f}, averaged_loss2: {:.6f}, averaged Loss: {:.6f}, tf_ratio: {:.6f}'.format(epoch, train_epoch_loss1, train_epoch_loss2, train_epoch_loss, teacher_forcing_ratio))
...
#                    break
                ...
                output, output1 = model(data, target, teacher_forcing_ratio=0)
                
                y_true.append(label)
                y_pred.append(output)
                #
                y_true_t.append(label1)
                y_pred_t.append(output1)
                
                output1 = output1.view(-1,24)
                _, predicted = torch.max(output1,-1)
                total += output1.cuda().size(0)
                correct += (predicted.cuda() == label1.cuda()).sum().item()
        ...
        if args.real_value:
            ...
            y_pred_t = scaler.inverse_transform(torch.cat(y_pred_t, dim=0))
        ...
        logger.info("time prediction accuracy is: {:.2f}%".format(acc))
    @staticmethod
